...
public class PageFunctionUtils {
	public static void clickOnElement(WebDriver driver, By by) {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofMillis(config.MAXTIME));
		wait.until(ExpectedConditions.elementToBeClickable(by));
		driver.findElement(by).click();
	}
	
	public static void enterDataInWebElement(WebDriver driver,By by,String keysToSend) throws Exception {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofMillis(config.MINTIME));
		wait.until(ExpectedConditions.elementToBeClickable(by));
	...
	public static void sleep() throws InterruptedException {
		Thread.sleep(Duration.ofMillis(config.SLEEPTIME));
		}
